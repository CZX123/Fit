...
class _MergeableMaterialState extends State<MergeableMaterial> with TickerProviderStateMixin {
  ...
  BorderRadius _borderRadius(int index, bool start, bool end) {
    assert(widget.borderRadius.topLeft == widget.borderRadius.topRight);
    assert(widget.borderRadius.topLeft == widget.borderRadius.bottomLeft);
    assert(widget.borderRadius.topLeft == widget.borderRadius.bottomRight);
    final Radius cardRadius = widget.borderRadius.topLeft;
  ...
  Widget build(BuildContext context) {
    ...
    if (slices.isNotEmpty) {
      widgets.add(
        new Container(
          decoration: new BoxDecoration(
            color: darkMode ? Colors.grey[850] : Colors.white,
            borderRadius: widget.borderRadius != BorderRadius.zero ? _borderRadius(i - 1, true, true) : BorderRadius.zero,
            shape: BoxShape.rectangle
          ),
          child: new ListBody(
            mainAxis: widget.mainAxis,
    ...
    return new _MergeableMaterialListBody(
      ...
      mainAxis: widget.mainAxis,
      boxShadows: widget.elevation == 0 ? <BoxShadow>[BoxShadow(color: Colors.transparent)] : kElevationToShadow[widget.elevation],
      items: _children,
      children: widgets
    );
  }
